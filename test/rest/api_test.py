...
class TestApi(unittest.TestCase):
    ...
    def test_api_multiply_2(self):
        url = f"{BASE_URL}/calc/multiply/3/0"
        response = urlopen(url, timeout=DEFAULT_TIMEOUT)
        self.assertEqual(
            response.status, http.client.OK, f"Error en la petición API a {url}"
        )
    ...
    def test_api_multiply_3(self):
        url = f"{BASE_URL}/calc/multiply/3/-1"
        response = urlopen(url, timeout=DEFAULT_TIMEOUT)
        self.assertEqual(
            response.status, http.client.OK, f"Error en la petición API a {url}"
        )
